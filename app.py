...
def consultar_duracao_ia(nome_obra):
    """
    Consulta gratuitamente a Wikidata para obter a duração da obra.
    O resultado alimenta o algoritmo geológico já existente.
    """
    endpoint = "https://query.wikidata.org/sparql"
    nome = nome_obra.replace("\\", "\\\\").replace('"', '\\"')

    queries = [
        f"""
        SELECT ?item ?itemLabel ?duration ?date WHERE {{
          ?item wdt:P2047 ?duration ;
                rdfs:label ?label .
          OPTIONAL {{ ?item wdt:P577 ?date . }}
          FILTER(LCASE(STR(?label)) = LCASE("{nome}"))
    ...
    headers = {
        ...
        "User-Agent": "PPCC-DGL7067-EscalaTempoGeologico/1.0"
    }
    ...
    for query in queries:
        try:
            ...
            for linha in resposta.json().get("results", {}).get("bindings", []):
                try:
                    minutos = float(linha["duration"]["value"])
                    if not (1 <= minutos <= 1440):
                        continue

                    titulo = linha.get("itemLabel", {}).get("value", nome_obra)
                    ano = None
                    if "date" in linha:
                        valor_data = linha["date"].get("value", "")
                        if valor_data:
                            ano = valor_data[:4]
                    ...
                    candidatos.append({
                        ...
                        "fonte": "Wikidata",
                        "url": linha.get("item", {}).get("value", ""),
                        "observacao": "Duração consultada na Wikidata. A conversão para tempo geológico é realizada pelo algoritmo do programa."
                    })
    ...
    if not candidatos:
        return {
            "ok": False,
            "erro": (
                "Não foi possível encontrar automaticamente uma duração confiável "
                "para essa obra na fonte consultada."
            )
        }
