...
def extrair_duracao_wikipedia(wikitext):
    """Tenta encontrar a duração no texto de uma página da Wikipédia."""
    # Exemplos aceitos: 127 minutos, 2 horas e 7 minutos, 2 h 7 min
    padroes = [
        r'\|\s*(?:dura[cç][aã]o|tempo de execu[cç][aã]o|running time)\s*=\s*([^\n<]+)',
        r'\|\s*(?:dura[cç][aã]o|tempo de execu[cç][aã]o|running time)\s*:\s*([^\n<]+)',
    ]
    valor = None
    for padrao in padroes:
        m = re.search(padrao, wikitext, flags=re.I)
        if m:
            valor = re.sub(r'<ref.*?</ref>', '', m.group(1), flags=re.I | re.S)
            valor = re.sub(r'\[\[.*?\]\]', '', valor)
            valor = valor.strip()
            break
    if not valor:
        return None
    m = re.search(r'(\d+)\s*h(?:oras?)?\s*(?:e\s*)?(\d+)?\s*min', valor, re.I)
    if m:
        return int(m.group(1)) * 60 + int(m.group(2) or 0)
    m = re.search(r'(\d+)\s*(?:minutos?|mins?)', valor, re.I)
    if m:
        return int(m.group(1))
    m = re.search(r'(\d+)\s*h(?:oras?)?', valor, re.I)
    if m:
        return int(m.group(1)) * 60
    return None


def consultar_duracao(nome_obra):
    """Consulta a Wikipédia, sem API key, e retorna duração + fonte."""
    headers = {"User-Agent": "PPCC-DGL7067/1.0 (ferramenta didatica)"}
    idiomas = ["pt", "en"]
    erros = []
    try:
        for idioma in idiomas:
            ...
            for item in resultados:
                ...
                r2 = requests.get(api, params=params2, headers=headers, timeout=15)
                r2.raise_for_status()
                data = r2.json()
                texto = data.get("parse", {}).get("wikitext", {}).get("*", "")
                minutos = extrair_duracao_wikipedia(texto)
                if minutos and 1 <= minutos <= 10000:
                    return {
                        "ok": True,
                        "obra": titulo,
                        "ano": "",
                        "duracao_minutos": minutos,
                        "fonte": f"Wikipédia ({idioma})",
                        "url": f"https://{idioma}.wikipedia.org/wiki/{requests.utils.quote(titulo.replace(' ', '_'))}",
                        "observacao": "A duração foi localizada automaticamente na ficha da obra. Confira a fonte se houver mais de uma versão ou corte da obra."
                    }
            erros.append(f"nenhuma duração encontrada na Wikipédia {idioma}")
        return {"ok": False, "erro": "Não foi possível encontrar automaticamente uma duração confiável para essa obra."}
    except requests.RequestException:
        return {"ok": False, "erro": "Não foi possível consultar a fonte de duração no momento. Verifique a conexão e tente novamente."}
    except Exception:
        return {"ok": False, "erro": "Ocorreu um problema ao consultar a duração da obra."}
...
if consultar:
    ...
    with st.spinner("Consultando a duração da obra..."): consulta=consultar_duracao(nome_obra)
    if not consulta["ok"]:
...
with tab_codigo:
    st.markdown("O código-fonte do artefato será disponibilizado nesta seção.\n\nA duração é consultada automaticamente em uma fonte pública. O cálculo da escala geológica é realizado pelo algoritmo local do programa.")
