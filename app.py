...
def consultar_duracao_ia(nome_obra):
    """
    Consulta gratuitamente a Wikidata.
    Primeiro procura o título; depois consulta diretamente as propriedades
    do item encontrado. O cálculo geológico continua sendo feito localmente.
    """
    ...
    try:
        ...
        # Se não houver resultado em português, tenta inglês.
        if not resultados:
            params["language"] = "en"
            params["uselang"] = "en"
            r = requests.get(search_url, params=params, headers=headers, timeout=12)
        ...
        if not resultados:
            return {
                "ok": False,
                "erro": "A obra não foi localizada na base consultada."
            }

        # 2) Escolhe o resultado mais diretamente correspondente ao título.
        nome_normalizado = nome_obra.strip().casefold()

        def qualidade(resultado):
            label = resultado.get("label", "").strip().casefold()
            desc = resultado.get("description", "").casefold()

            pontos = 0
            if label == nome_normalizado:
                pontos += 100
            elif nome_normalizado in label:
                pontos += 40

            if "film" in desc or "filme" in desc:
                pontos += 20
            if "television" in desc or "television" in label or "série" in desc:
                pontos += 10

            return pontos

        resultados.sort(key=qualidade, reverse=True)
        qid = resultados[0].get("id")

        if not qid:
            return {
                "ok": False,
                "erro": "A consulta encontrou resultados, mas não conseguiu identificar a obra."
            }

        # 3) Busca as propriedades completas do item.
        params = {
        ...
        r = requests.get(entity_url, params=params, headers=headers, timeout=12)
        r.raise_for_status()
        ...
        # P2047 = duração.
        duracoes = claims.get("P2047", [])

        if not duracoes:
            return {
                "ok": False,
                "erro": (
                    f"A obra foi localizada ({qid}), mas não há duração "
                    "cadastrada para ela na fonte consultada."
                )
            }

        candidatos = []

        for claim in duracoes:
            mainsnak = claim.get("mainsnak", {})
            datavalue = mainsnak.get("datavalue", {})
            valor = datavalue.get("value", {})
            ...
            # P2047 é uma quantidade. Normalmente a unidade é minuto,
            # mas conferimos a unidade para evitar interpretar outra coisa.
            ...
            if "Q7727" in unit:
                ...
                # Algumas quantidades podem aparecer sem unidade explícita.
                # Para P2047, tratamos o valor como minutos.
                minutos = amount
            else:
                continue
        ...
        if not candidatos:
            return {
                "ok": False,
                "erro": "A obra foi localizada, mas a duração cadastrada não possui uma unidade reconhecível."
            }

        minutos = round(candidatos[0])

        labels = entity.get("labels", {})
        label = (
            labels.get("pt", {}).get("value")
            or labels.get("en", {}).get("value")
            or resultados[0].get("label")
            or nome_obra
        )

        descricao = (
            entity.get("descriptions", {}).get("pt", {}).get("value")
        ...
        return {
            ...
            "observacao": (
                "Duração consultada na Wikidata. "
                "A conversão para a escala de tempo geológico é realizada "
                "exclusivamente pelo algoritmo do programa."
            )
        }
